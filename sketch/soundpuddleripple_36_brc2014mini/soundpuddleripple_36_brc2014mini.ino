...
int shiftdelay = 15; // delay in mS to slow down the shift interpolation function

// Specifiy interpolation function variables
#define stepcount 3

// HSV color space controls
float hue_offset = 0.2; // phase shift for the HSV function (range 0.00-0.99)
float hue_limiter = 0.25;
float hue_multiplier = 1.0;
float hsvalue_max = 0.3;
...
unsigned fftbuffermap[BUFFERSIZE]= {14,15,16,17,18,19,20,22,23,24,26,27,29,31,33,35,37,39,41,44,46,49,52,55};
...
void genhsvtable(float hue_offset) {
  ...
  for (i=0;i<256;i++) {
    ...
    hsvalue = (((float)i)-(float)hsvalue_floor)/350;
...
void loop()
{
	...
	if (samp_done == 1) {
	  ...
	  for (z=0; z<BUFFERSIZE; z++) {
	    ...
	    if (val < buffermapoffset[z]) {val = 0;}
// 	    Serial.print(val);
// 	    Serial.print(",");
	    val = val/255;
