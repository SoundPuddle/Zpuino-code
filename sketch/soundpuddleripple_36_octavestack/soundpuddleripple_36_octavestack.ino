...
signed r_slope[BUFFERSIZE/3]; 
signed g_slope[BUFFERSIZE/3];
signed b_slope[BUFFERSIZE/3];
...
void loop()
{
	...
	if (samp_done == 1) {
	  ...
	  // first we will go through the input buffer, and for every third element draw a new HSV pixel
	  for (z=0; z<BUFFERSIZE; z++) {
	    if (primary_counter == 0) {
	      ...
	      bin_val_new[z] = val;
	      outbuffer[z+1] = hsvtable[bin_val_old[z]];
	      primary_counter++;
	    }
	    if (primary_counter > 2) {primary_counter = 0;}
	    }
	    
	  // Now we will go through the input buffer again and calculate the RGB slope between spokes
	  int rtrans0, gtrans0, btrans0;
	  for (z=0; z<BUFFERSIZE/3; z++) {
	    rtrans0 = (outbuffer[i] >> 24) - 128;
	    gtrans0 = ((outbuffer[i] & 0x00ff0000) >> 16) - 128;
	    btrans0 = ((outbuffer[i] & 0x0000ff00) >> 8) - 128;
	    rtrans1 = (outbuffer[i+BUFFERSIZE] >> 24) - 128;
