...
int shiftdelay = 8; // delay in mS to slow down the shift interpolation function

// Specifiy interpolation function variables
#define stepcount 3

// HSV color space controls
float hue_offset = 0.61; // phase shift for the HSV function (range 0.00-0.99)
//float hue_limiter = 0.25;
int hue_divisor = 600; // Nominal value is 255
...
void genhsvtable(float hue_offset) {
  ...
  for (i=0;i<256;i++) {
    ...
    hue = 1-((((float)i)/hue_divisor)+hue_offset);
    if (hue < 0) {hue = 0;}
