...
{16,17,18,19,20,21,23,24,25,27,29,30,32,34,36,38,41,43,46,48,51,54,58,61,65,69,73,77,82,87,92,97,103,109,116,123,130,138,146,155,164,174,184,195,207,219,232,246,261,276,293,310,329,348,369,391,414,439,465,493,522,553,586,621,658,697,739,782,829,878,931,986};
...
//	outbuffer[0] = 0;
	...
	for (z=0; z<NUMSTRIPS; z++) { 
	  i = 0;
	  for (x=0; x < OCTAVE_SIZE; x++) { //for a given note in each octave
		i += fftbuffermap[z * x]; //add the value of each successive note to the total
	  Serial.print(fftbuffermap[z * x]);
	  Serial.print(";");
	  Serial.print(i);
	  Serial.println();
	  }
	  i = i / OCTAVE_SIZE; //calculate the average value of the sum of the octaves (we could change this to a log function, or some other function)
